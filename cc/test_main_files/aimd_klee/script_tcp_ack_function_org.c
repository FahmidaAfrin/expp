...
int main() 
{
	...
	for (u32 i=0; i<num_events; i++) {
		...
		else if (which_flag[i] == 1) { // losses
			if (num_loss_events > 0)
				flag = FLAG_LOST_RETRANS;
			else
				flag = FLAG_DATA;
		}
		else if (which_flag[i] == 2) {// Undoing by goto no_queue in tcp_ack()
			if (tp->undo_retrans == 0)
				flag = FLAG_DSACKING_ACK;
			else
				flag = FLAG_DATA;
		}
		else if (which_flag[i] == 3) { // timeout
			if (num_loss_events > 0)
...
//			__VERIFIER_assert(tp->snd_cwnd >= prior_loss_cwnd);
		...
		else if (flag & FLAG_SET_XMIT_TIMER) { // Timeout
